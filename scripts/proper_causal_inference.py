...
===========================
Corrected Causal Inference Framework — Customer Risk & Retention
Portfolio Project | Google GBS&O Alignment
...
Purpose:
    ...
    tenure, contract type, and payment method as confounders. This script:
      ...
      6. Produces interview-ready output and visualisations

    This is the differentiating section of the portfolio — catching and
    correcting a common methodological error signals top-tier causal thinking.
...
Google Role Alignment:
    - Causal inference expertise (job requirement)
    - Intellectual honesty / critical thinking (BizOps value)
    - Translating ambiguous analytical findings into clear strategy
    - Policy, Risk & Compliance framing (churn = customer risk)
"""
...
def compare_wrong_vs_correct_model(df: pd.DataFrame):
    ...
    """
    print("\n" + "=" * 70)
    print("STEP 3: WRONG MODEL vs. CORRECT MODEL — SIDE-BY-SIDE")
    print("=" * 70)
    print("  WRONG  (v1): controls for tenure, contract, payment, charges")
    ...
    print(f"  {'WRONG v1 (endogenous controls)':<30}  {wrong_samples.mean()*100:>+7.1f}pp"
    ...
    print("""
  Key insight:
    The WRONG model over-adjusts because tenure is a collider — controlling
  ...
  What this means for strategy:
    ...
    ax.hist(wrong_samples * 100,   bins=bins, alpha=0.6, color='#D32F2F',
            label='WRONG v1 (endogenous controls)', density=True)
    ...
    ax.set_title('Wrong vs. Correct Causal Model — Add-on Effect Posterior\n'
...
# =============================================================================
# STEP 6: SUMMARY AND INTERVIEW PREP
# =============================================================================
...
def generate_summary_and_interview_prep(df, wrong_samples, correct_samples):
  ...
  Q: "How did you handle confounding in this project?"
  A: "I had to identify which variables were valid confounders and which
     weren't. Tenure looked like a natural control variable, but I realised
     it's actually a collider — it's caused by both add-on adoption and
     churn. Conditioning on it opens a spurious path and induces survivorship
     bias. I demonstrated this empirically: the add-on effect reverses among
     long-tenure customers, which is a textbook collider signature, not
     real heterogeneity. I also identified that contract type, payment method,
     and monthly charges are other treatments or mediators — not confounders.
     After removing them, I only controlled for exogenous demographics:
     age, dependents, and partner status."

  Q: "What did you do after catching the error?"
  A: "I documented it explicitly in the portfolio. Catching and correcting a
     methodological error is more valuable than never making one — it shows
     critical thinking and intellectual honesty. I then proposed a 2×2×2
     factorial randomised experiment as the definitive solution, since
     randomisation eliminates all selection bias from endogenous choices
     that observational methods cannot handle."

  Q: "Why not just use propensity score weighting?"
  A: "Propensity score weighting adjusts for observed confounders, but the
     key confounders here — customer loyalty and satisfaction — are
     UNOBSERVED in the dataset. PSW can't fix what it can't measure.
     Randomisation is the only method that removes bias from both observed
     and unobserved confounders. That's why the experiment is the right
     answer here."

  Q: "What is a collider and why does it matter?"
  A: "A collider is a variable that is caused by two or more variables in
     a causal graph. Conditioning on a collider — including it in a
     regression — opens a spurious association between its causes.
     In this case, tenure is caused by both add-on adoption (add-ons may
     retain customers longer) and churn (churned customers have low tenure).
     Controlling for tenure in a model of add-ons and churn mixes up these
     causal pathways and produces a biased estimate. Most analysts don't
     check for this — it's a differentiating skill."
    """)

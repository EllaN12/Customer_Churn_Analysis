...
def calculate_sample_size(effect_size, alpha=0.05, power=0.8, baseline=None):
    """
    Approximate sample size calculation for proportion test.
    baseline defaults to None — callers must pass the segment-specific observed churn rate.
    Do NOT hardcode 0.715 (old ML model mean) or 0.265 (full-population rate).
    """
    if baseline is None:
        baseline = 0.747  # fallback: early-tenure (0–6 months) observed rate
...
def main():
    ...
    # ── Approved Test Presets ─────────────────────────────────────────────────
    # Each preset loads the segment-specific OBSERVED churn rate as the baseline
    # (NOT the full-population 26.5% and NOT the ML model mean predicted prob 71.4%)
    st.sidebar.subheader("Quick Presets — Approved Tests")
    PRESETS = {
        "Custom (manual)": {
            "baseline": 74.7, "effect": 10, "n_arms": 4, "arms": 4,
            "desc": "Configure manually below"
        },
        "Test 1 — Early Tenure (0–6 mo)": {
            "baseline": 74.7, "effect": 10, "n_arms": 4, "arms": 4,
            "desc": "Segment baseline: 74.7% (observed 0–6 month cohort). "
                    "4-arm: Control / Welcome Call / Smart Start / Concierge."
        },
        "Test 2 — Contract + AutoPay (MTM / e-check)": {
            "baseline": 62.0, "effect": 8, "n_arms": 3, "arms": 3,
            "desc": "Segment baseline: ~62% (observed month-to-month + electronic check). "
                    "3-arm: Control / Contract offer / AutoPay offer."
        },
        "Test 3 — Service Upsell (Fiber, <2 add-ons)": {
            "baseline": 55.0, "effect": 8, "n_arms": 3, "arms": 3,
            "desc": "Segment baseline: ~55% (Fiber customers with fewer than 2 add-ons). "
                    "3-arm: Control / OnlineSecurity / TechSupport."
        },
    }
    preset_choice = st.sidebar.selectbox("Load a preset", list(PRESETS.keys()))
    preset = PRESETS[preset_choice]
    if preset_choice != "Custom (manual)":
        st.sidebar.info(preset["desc"])
    ...
    baseline_churn = st.sidebar.slider(
        ...
        min_value=20,
        max_value=90,
        value=int(preset["baseline"]),
        step=1,
        help="Segment-specific OBSERVED churn rate. "
             "Use a preset above, or enter the actual historical rate for your target cohort."
    ) / 100
    ...
    with tab2:
        ...
        for arm in range(1, n_arms):
            ...
            # Decision — design spec thresholds: P > 0.95, min 8pp
            if pos > 0.95 and ev_samples.mean() > 50:
                decision = "✅ IMPLEMENT"
                decision_class = "success"
            elif in_rope > 0.95:
                decision = "≈ ROPE EQUIVALENT"
        ...
        # Interpretation
        st.info("""
        **Decision Criteria (design spec):**
        - ✅ **IMPLEMENT**: P(reduces churn >8pp) > **95%** AND Expected Value > $50
        - ≈ **ROPE EQUIVALENT**: P(within ±2pp) > 95% — no practically meaningful difference
        - ⚠️ **PROMISING**: P(Superior) > 80% — keep monitoring
    ...
    with tab4:
        ...
        **Key Insight:** As we collect more data, the posterior (solid line)
        ...
        # ── ROPE Visualization ────────────────────────────────────────────────
        st.markdown("---")
        st.subheader("ROPE Analysis — Region of Practical Equivalence")
        st.markdown(
            "ROPE = **±2 percentage points**. "
            "If the posterior difference between treatment and control is almost entirely "
            "within (−0.02, +0.02), the treatments are **practically equivalent** "
        ...
        if n_arms >= 2:
            ...
            fig_rope.add_trace(go.Scatter(
                ...
                name='Posterior difference (Control − Treatment)',
                line=dict(color='steelblue', width=2)
            ))

            # ROPE band
            rope_mask = (diff_x >= rope_low) & (diff_x <= rope_high)
            fig_rope.add_trace(go.Scatter(
                x=np.concatenate([[rope_low], diff_x[rope_mask], [rope_high]]),
                y=np.concatenate([[0], diff_y[rope_mask], [0]]),
                mode='lines', fill='tozeroy', opacity=0.5,
                name=f'ROPE (±2pp) — {pct_in_rope:.1%} of mass',
                line=dict(color='orange', width=0),
                fillcolor='rgba(255,165,0,0.4)'
            ))

            fig_rope.add_vline(x=0,     line_dash='dash', line_color='gray',
                               annotation_text='No difference')
            fig_rope.add_vline(x=0.08,  line_dash='dot',  line_color='green',
                               annotation_text='Min meaningful effect (8pp)')
            fig_rope.add_vline(x=rope_low,  line_dash='solid', line_color='orange', line_width=1)
            fig_rope.add_vline(x=rope_high, line_dash='solid', line_color='orange', line_width=1)

            fig_rope.update_layout(
                title='Posterior Distribution of Churn Reduction (Control − Treatment 1)',
                xaxis_title='Churn Rate Difference (percentage points)',
                yaxis_title='Density',
                height=420,
                hovermode='x unified'
            )
            ...
            r1.metric("P(Meaningfully better >8pp)", f"{pct_above_rope:.1%}",
                      delta="→ evidence for implementation" if pct_above_rope > 0.95 else None)
            r2.metric("P(Within ROPE ±2pp)",          f"{pct_in_rope:.1%}",
                      delta="→ practically equivalent" if pct_in_rope > 0.95 else None)
            r3.metric("P(Worse than control)",        f"{pct_below_rope:.1%}")

"""
Power Analysis for Randomized Experiments (UPDATED)
Uses config.py for directory management
...
from config import (
    ...
    POWER_ANALYSIS_CONFIG, EARLY_TENURE_CONFIG, FACTORIAL_CONFIG,
...
class ExperimentPowerAnalysis:
    ...
    def __init__(self):
        ...
        self.n_simulations = POWER_ANALYSIS_CONFIG['n_simulations']
        self.alpha = POWER_ANALYSIS_CONFIG['alpha']
        
        print(f"✓ Power analysis initialized")
        print(f"  Results directory: {results_dir}")
        print(f"  Visualizations: {visualizations_dir}")
    
    def simulate_experiment_1_early_tenure(self, n_per_arm=100, n_simulations=None):
        """
        Experiment 1: Early Tenure Intervention
        ...
        true_rates = {
            'Control': EARLY_TENURE_CONFIG['baseline_rate'],
            'T1_WelcomeCall': 0.509,
            'T2_SmartStart': 0.459,
            'T3_Concierge': 0.409
        }
        ...
        for sim in tqdm(range(n_simulations)):
            ...
            for arm in ['T1_WelcomeCall', 'T2_SmartStart', 'T3_Concierge']:
                ...
                prob_superior = (diff > 0.10).mean()  # >10% reduction
                
                bayesian_results[arm] = {
                    'prob_superior': prob_superior,
                    'detected': prob_superior > 0.90
                }
            ...
            for arm in ['T1_WelcomeCall', 'T2_SmartStart', 'T3_Concierge']:
                ...
                freq_results[arm] = {
                    'p_value': p_value,
                    'detected': p_value < self.alpha and sim_data[arm]['rate'] < sim_data['Control']['rate']
                }
    ...
    def simulate_experiment_2_factorial(self, n_per_cell=100, n_simulations=None):
        """
        ...
        for sim in tqdm(range(n_simulations)):
            ...
            contract_detected = (mtm_samples - contract_samples > 0.15).mean() > 0.90
            ...
            payment_detected = (current_samples - autopay_samples > 0.10).mean() > 0.90
    ...
    def sample_size_sensitivity(self, experiment='early_tenure'):
        ...
        sample_sizes = POWER_ANALYSIS_CONFIG['sample_sizes_to_test']
        power_results = []
        ...
        save_path = get_visualization_path('sample_size_sensitivity.png')
    ...
    def _plot_power_comparison(self, df_power, experiment_name):
        ...
        # Save using config path
        save_path = get_visualization_path('power_comparison.png')
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        print(f"\n✓ Saved: {save_path}")
        plt.close()

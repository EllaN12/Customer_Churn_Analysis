...
## Based on Full Dataset Analysis (N=1,521)
...
**Dataset Profile:**
- Total high-risk customers: 1,521 (100% predicted to churn)
- Actual churn rate: 71.5% (1,087 churned, 434 retained)
- Current state: 99.9% month-to-month contracts
- Mean predicted churn probability: 66.8% (range: 50%-91%)
- Average monthly revenue: $78.76
- Critical insight: **73.2% use electronic check** (highest churn method)

**Revenue at Stake:**
- Monthly revenue at risk: $119,908 (1,521 × $78.76)
- Annualized churn loss: ~$1.03M if 71.5% churn at average 11-month LTV
...
**Why This Test First:**
- 99.9% are month-to-month (universal pain point)
- 73.2% use electronic check (switch to auto-pay = proven retention lever)
- Combined intervention likely has multiplicative effect

**Sample Allocation:**
- **Total sample:** 800 customers (52% of dataset)
- **Stratified by:** Risk tier (extreme/high/moderate) + Tenure group

**Treatment Arms (n=200 each):**

| Arm | Intervention | Expected Effect |
|-----|-------------|----------------|
| **Control** | Status quo | Baseline (71.5% churn) |
| **T1: Standard Contract** | 1-year at 10% discount | -12% churn (59.5%) |
| **T2: Contract + AutoPay** | 1-year at 10% + autopay incentive ($10 credit) | -20% churn (51.5%) |
| **T3: Premium Bundle** | 2-year at 20% + autopay + tech service bundle | -28% churn (43.5%) |

**Bayesian Priors:**
```python
# Based on industry benchmarks and payment method analysis
...
with pm.Model() as contract_model:
    ...
    μ_baseline = pm.Normal('mu_baseline', mu=0.7, sigma=0.1)
    
    # Risk tier effects (extreme, high, moderate)
    τ_risk = pm.HalfNormal('tau_risk', sigma=0.1)
    risk_effect = pm.Normal('risk_effect', mu=0, sigma=τ_risk, shape=3)
    
    # Treatment effects
    τ_treatment = pm.HalfNormal('tau_treatment', sigma=0.15)
    treatment_effect = pm.Normal('treatment_effect', mu=[-0.12, -0.20, -0.28], 
                                  sigma=τ_treatment, shape=3)
    
    # Segment-specific baselines
    p_churn = pm.Deterministic('p_churn',
        pm.math.invlogit(
            pm.math.logit(μ_baseline) + risk_effect[risk_tier] + treatment_effect[treatment]
        )
    )

...
Portfolio Project: Customer Churn & Retention Analysis
...
class FullDatasetCausalAnalysis:
    ...
    Key specification:
      ...
      - These are colliders / other treatments / mediators 
...
but because of selection.
  ...
  Correct conclusion:
    ...
    # ── 3. Causal analysis #1 — early tenure (exogenous confounders only) ─────────────────────
    def analyse_early_tenure(self):
        """
        Causal effect of early tenure on churn.
        Model specification: only exogenous confounders (senior, dependents, partner).
    ...
    def analyse_addons(self):
        ...
        print("=" * 70)
        print("  Removed from model: tenure_std, is_mtm, is_fiber, charges")
        ...
        print(f"  Causal estimate        : {samples.mean()*100:+.1f}pp")
        print(f"  95% Credible Interval     : [{np.percentile(samples,2.5)*100:.1f}pp,"
              f" {np.percentile(samples,97.5)*100:.1f}pp]")
    ...
    def create_summary_report(self):
        """Save causal validation summary to CSV"""
        print("\n" + "=" * 70)
...
if __name__ == '__main__':
    ...
    print("  causal_validation_summary.csv — structured output ")
    print()
    print("  Key findings:")

...
class BayesianExperimentSimulator:
    ...
    def plot_power_analysis(self, df_results, save_path='./'):
        """
        ...
        for _ in range(100):
            ...
            sample_sims.append(probs_over_time)
        
        # Plot mean and confidence bands
        mean_probs = np.mean(sample_sims, axis=0)
        std_probs = np.std(sample_sims, axis=0)
        ...
        plt.tight_layout()
        plt.savefig(f'{save_path}power_analysis_results.png', dpi=300, bbox_inches='tight')
        print(f"✓ Saved: {save_path}power_analysis_results.png")
        plt.close()
...
def compare_sample_sizes(true_effects, baseline_churn=0.715):
    ...
    plt.tight_layout()
    plt.savefig('/mnt/user-data/outputs/sample_size_comparison.png', dpi=300, bbox_inches='tight')
    print(f"\n✓ Saved: sample_size_comparison.png")
...
if __name__ == '__main__':
    ...
    simulator.plot_power_analysis(df_results, save_path='/mnt/user-data/outputs/')
...
"""
    ...
    with open('/mnt/user-data/outputs/simulation_summary_report.md', 'w') as f:
        f.write(summary_report)
    
    print("✓ Summary report saved: simulation_summary_report.md")
    
    # Save detailed results
    df_results.to_csv('/mnt/user-data/outputs/simulation_detailed_results.csv', index=False)
    print("✓ Detailed results saved: simulation_detailed_results.csv")
